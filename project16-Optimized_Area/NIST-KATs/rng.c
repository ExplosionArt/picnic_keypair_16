...
AES256_ECB(unsigned char *key, unsigned char *ctr, unsigned char *buffer)
{
    ...
    //KeyExpansion(ct->RoundKey,(const uint8_t*) key);
    ...
    //Cipher((state_t*)test, *ctx->RoundKey);
    for(i = 0; i < 4; i++)
    {
		for(j = 0; j < 4; j++)
		{
...
randombytes(unsigned char *x, unsigned long long xlen)
{
    ...
    //Replacing the function call by definition//
    unsigned char *provided_data = NULL, *Key = DRBG_ctx.Key, *V = DRBG_ctx.V;
